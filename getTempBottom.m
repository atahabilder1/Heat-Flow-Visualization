...
function TempBottom = getTempBottom
% Program Name: Generating a Time vs Temperature movie  

% Get user input for the TempBottom
done = false;
while(~done)
    TempBottom=input('Enter the Temperature (in deg C) of the bottom probe: ');% To take the input
    if (TempBottom < -273 || TempBottom > 10000)  % to check weather it in between range or not
